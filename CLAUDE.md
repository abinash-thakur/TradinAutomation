...
**A short call expiring today is never bought back — it's left to expire naturally, and never counts as coverage.** `isExpiringToday()` (6-digit `DDMMYY` code from the symbol or the stored `expiryDate`) filters it out of both the held-call count and the strike reference, in the engine *and* the preview, so a routine trigger can short a fresh next-day call while today's expiring call is still open — the two coexist briefly rather than the engine closing one to make room for the other. (An earlier version of this engine *did* buy back today's expiring call before shorting the next one — that "roll" behavior was deliberately removed; if you're tempted to re-add it, confirm that's actually wanted first, it was a real decision, not an oversight.)
