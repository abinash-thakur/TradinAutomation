...
docker compose up -d
```
Both Dockerfiles' `pnpm install` use a BuildKit cache mount (`--mount=type=cache,target=/root/.local/share/pnpm/store`) specifically so a killed/retried build doesn't re-download everything from zero — Docker only commits a *layer* on success, so without this a partial `pnpm install` (OOM, dropped connection, anything) throws away every package already fetched; the cache mount persists on the host independent of that.
