...
Both apps need a `.env` copied from `.env.example`. The backend requires a reachable **remote** PostgreSQL and Redis — there is no local/docker DB setup and no migrations (`synchronize: true` in [app.module.ts](backend/src/app.module.ts)), so schema changes come from editing entities. [database-seed.service.ts](backend/src/config/database-seed.service.ts)'s `onModuleInit()` runs on **every boot**, not just the first — a Delta template broker account and BTC/ETH covered-call strategies are seeded only if none exist (guarded by `brokerCount === 0` / `!hasBtc` / `!hasEth`), but a trailing backfill loop over every existing strategy used to unconditionally overwrite `name` and `exitRules` back to hardcoded defaults on every single restart, silently reverting any value the user had actually configured (e.g. a custom `exitRules.optionProfitTargetPercent`). That loop is now guarded to only fill in a field that's genuinely missing — never touches one that's already set. If you add another "backfill old rows" step here, make it conditional too, or it'll clobber real user data every deploy.
...
docker compose down
```
On a small/constrained server, build one service at a time instead — `docker compose up -d --build` (via `docker buildx bake`) builds every service in parallel by default, so backend's and frontend's `pnpm install` (923 packages each) compete for the same CPU/RAM/bandwidth simultaneously, which is enough to trigger an OOM kill (`failed to execute bake: signal: killed`) on a small instance. Also run it detached from the SSH session (`tmux`/`screen`/`nohup ... &`) — a build like this can take several minutes on a modest server, and a dropped SSH connection kills the foreground process along with it:
```bash
docker compose build backend
docker compose build frontend
docker compose up -d
```
Deliberately **no** BuildKit cache mount on `pnpm install` in either Dockerfile — a cache mount (`--mount=type=cache,target=...`) would survive a killed/retried build without re-downloading everything, but it does that by permanently reserving disk on the host, which is the wrong tradeoff on a disk-constrained box (this app has shipped on an AWS instance with an 8GB root volume already at 92% used). Re-fetching on retry costs bandwidth; a cache mount costs disk you may not have. Only add one back if the deploy target has disk to spare.
Requires `backend/.env` to already exist (`env_file:` in [docker-compose.yml](docker-compose.yml) — there's no root `.env`, and the backend still needs the same remote Postgres/Redis reachable from wherever the container runs). [backend/Dockerfile](backend/Dockerfile) is Debian slim, not alpine — `sqlite3` and `bufferutil` are optional native peers of TypeORM/socket.io that pnpm compiles from source (`allowBuilds` in `pnpm-workspace.yaml`), and alpine's musl libc is a common source of native-module breakage; the `deps` stage installs `python3 make g++` for that compile step. **`pnpm-workspace.yaml` must be copied into the image alongside `package.json`/`pnpm-lock.yaml`** before `pnpm install` — it's what carries the `allowBuilds` allowlist, and without it pnpm silently blocks those same build scripts (`ERR_PNPM_IGNORED_BUILDS`) instead of failing loudly.

There is deliberately only **one** `pnpm install` in the backend build (the `deps` stage, with dev deps) — the production `node_modules` is derived from it via `pnpm prune --prod` in a stage that inherits from `deps`, not a second independent `pnpm install --prod`. Docker runs independent build stages in parallel, so two separate installs would fetch the same large packages (`ccxt`, `@swc/core`'s native binary) at the same time on two connections, competing for bandwidth for no reason — very noticeable on a small/bandwidth-limited server. If you ever split dependency installation into more than one stage again, make sure they're not sibling stages that both hit the network.
...
- **Order placed** — `StrategyEngineService.logTrade()` (every real order fill runs through here already) fires `notifyOrderPlaced()` for a whitelisted action set (`BUY_FUTURE_ENTRY`, `SELL_CALL_ENTRY`, `BUY_FUTURE_AVERAGING`, `SELL_CALL_BEST_PREMIUM`). `*_FAILED` and `ROLLBACK_*` rows do **not** email — only successful placements do. `BUY_CLOSE_DECAYED_CALL` (the decay roll's close leg) is deliberately **not** whitelisted either — it's a close, not a placement; its outcome shows up in the following `SELL_CALL_BEST_PREMIUM` email's body text instead of firing its own email.
...
Everything trading-related goes through `IBrokerAdapter` in [broker.interface.ts](backend/src/brokers/broker.interface.ts). Implementations: `DeltaExchangeAdapter` (India/Global, one class flagged by constructor arg), `CcxtBrokerAdapter` (binance/bybit/deribit). The engine never imports an adapter directly — add new venues by implementing the interface and adding a `case` in [broker-factory.service.ts](backend/src/brokers/broker-factory.service.ts).

**No paper/simulated broker exists** — every `BrokerAccount` is a real venue. `BrokerFactoryService.getAdapter()` **throws** for any `brokerType` it doesn't recognize (including `'zerodha'`, which is declared in the `BrokerType` union but has no adapter implemented) rather than silently substituting anything — trading somewhere the user didn't explicitly choose is worse than a loud failure. This was a deliberate removal: the factory used to fall back to a `PaperTradingAdapter` for any unrecognized type, which meant a strategy could silently paper-trade with zero indication anything was wrong. `PaperTradingAdapter` ([adapters/paper-trading.adapter.ts](backend/src/brokers/adapters/paper-trading.adapter.ts)) still exists and is still used — but only internally, by `StrategyService.testScript()` for the sandbox's dry-run "Test Script" feature against simulated data, never as a selectable `BrokerAccount`.
...
**`getAdapterForStrategy()` throws if `strategy.brokerAccountId` doesn't resolve to an existing row** (`"Broker account <id> not found for strategy <name>"`) — there's no FK constraint enforcing referential integrity between `strategies.brokerAccountId` and `broker_accounts.id`, so a strategy can end up pointing at a broker account that was since deleted, and this is how that surfaces at trigger time.
...
- **Future exists, drawdown ≤ -1.0%** from `state.buyingStrike` → averaging layer (`executeDrawdownAveraging`), which re-bases `buyingStrike` to the new weighted-average entry so the next layer needs a further 1% drop. Always next-day ATM, unaffected by the best-premium logic below.
- **Future exists, drawdown > -1.0%** → routine call sell (`executeRoutineCallSell`), gated by a rule that runs *before* the ratio cap: it only actually sells when there is **no held call, or the held call expires today** — while a longer-dated call (from the decay roll below) is still open, every trigger is a no-op ("holding, no new order") until that call's own expiry day arrives. When it does sell, the ratio cap still applies (`futureLots * legsConfig.maxCallsPerFutureLot`, default 1 = strict 1:1, higher = a ratio write where calls beyond the line are **naked**, logged as `RATIO WRITE`), and the strike/expiry are chosen by `findBestPremiumCallThisWeek()` (see below) rather than always next-day ATM.
- **Monitor loop** (`monitorActivePositions()`, every 10s): two independent continuous checks, in this order —
  1. **Option profit-decay roll** (`executeOptionDecayRoll`, default `exitRules.optionProfitTargetPercent` = 75%): once the held call's `pnlPercent` (how much of its entry premium has decayed away) crosses this, buy it back and immediately short a fresh call at the *current* ATM strike (recomputed, not the old strike) via `findBestPremiumCallThisWeek()`. Only the option leg changes; the future is untouched. Non-fatal on either leg failing — see the method's own docstring for exactly how it degrades.
  2. **Future profit target** (`exitRules.futureProfitTargetPercent`, default 4%): closes everything (`emergencySquareOff`) and skips all other logic for that strategy this tick.

**`findBestPremiumCallThisWeek()`** is the shared strike/expiry selection used by both the routine call sell and the decay roll: at a given ATM strike, it checks up to 7 daily expiries (Delta settles daily) and picks whichever pays the highest premium — not a fixed offset. It guards against `OptionsUtil.findMatchingCall`'s documented fallback-to-full-chain behavior by verifying the returned contract's own expiry code actually matches the day requested, so a day that doesn't exist on the exchange can't get silently misattributed and win the comparison by mistake.
...
**A short call expiring today is never bought back by the routine — it's left to expire naturally, and never counts as coverage.** (It *is* bought back by the decay roll above, but only when premium-decay triggers that, never purely because of the calendar date.) `isExpiringToday()` (6-digit `DDMMYY` code from the symbol or the stored `expiryDate`) filters an expiring-today call out of both the held-call count and the strike reference, in the engine *and* the preview.

`getTriggerPreview()` mirrors the routine's gate and `findBestPremiumCallThisWeek()` selection, but has no equivalent for the decay roll — that's a continuous background check, not part of the scheduled trigger, so there's nothing to preview. **Any change to entry/averaging/routine logic must be made in both `executeRoutineCallSell()` and `getTriggerPreview()` or the preview lies to the user.**
...
One row holds `triggerConfig`, `indicatorConfig`, `legsConfig`, `exitRules`, and mutable `state` as `jsonb`. `state` is the source of truth for open legs, `buyingStrike`, drawdown, and realized PnL. `state.lastRollCount` is vestigial — it supported a "roll" (buy back today's expiring call before shorting the next one) that has since been removed; nothing sets it anymore, a couple of `rollCount` reads still fall back to it defensively. `logicMode` is `'CODE' | 'VISUAL_RULES'`, but the scheduler path always runs the hardcoded engine — `customScript` is currently only executed through the sandbox test endpoint.
...
**A form-initialization `useEffect` must key on `strategy?.id`, never on the whole `strategy` object.** `PrebuiltStrategyPanel`'s settings form (broker selection, lot sizes, margin mode, exit rules, schedule) used to re-initialize on every change to the `strategy` prop — and a live `strategy-update` socket event replaces that object on every scheduled trigger and every 10s monitor tick, whether or not anything the form cares about actually changed. The effect fired on every one of those broadcasts and silently reset every field in the form (broker dropdown included) back to the last-saved server value, discarding any in-progress edit before the user got to click Save. Keying on `strategy?.id` instead means it only re-initializes when the user switches to editing a genuinely different strategy — the same trap applies to any other form fed by a `strategy`/`strategies` prop that receives live socket updates.
